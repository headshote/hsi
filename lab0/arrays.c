...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer:
  // Nothing really, program just tries to access an address in memory of sizeof(int)
  // that is supposed to be located after memory address of array[9], which may or may not result
  ...
  // Answer:
  // Because arrays "work" like pointers or vise versa, because type of result of &varofsometype operation
  // is of type sometype* and our fisrt argument is pointer to an int;
  // and because we are passing value here by reference, hence that function can modify whatever data is at that
  // address (&value)
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer:
	// For the run where we don't free() anything we'll get
	// LEAK SUMMARY:
	// ==3038==    definitely lost: 20 bytes in 1 blocks
  ...
  // variable instead of the . operator).
	FourInts*  fi = malloc( sizeof(FourInts) );
	fillArray( (int*) fi, 4 );
	assert(fi->a == 2);
  assert(fi->b == 5);
  assert(fi->c == 8);
